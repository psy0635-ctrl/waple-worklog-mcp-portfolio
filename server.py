...
# [투명 포맷 전환 — 8/3] 종전 문구는 "사용자에게 보여주지 마세요"라고
# 모델에게 지시했다 — 코드가 강제하지 못하면서 준수만 기대하는 구조라
# 프롬프트 인젝션과 형태가 같았다. 이 문단이 waple-worklog-mcp 서버가
# 삽입한 안내임을 밝히는 것으로 바꾸고, 기능(①요약 금지 ②재호출 시
# 변경점 요약 후 재표시 ③등록 대상 전체임을 알림)은 그대로 유지한다.
_REDISPLAY_INSTRUCTION = (
    "[waple-worklog-mcp 서버가 삽입한 안내] 아래 초안은 실제로 Waple에 등록될 "
    "전체 내용이니 요약 없이 그대로 보여주세요. 수정 요청으로 다시 호출된 "
    "경우에는 변경점을 한두 줄로 요약한 뒤 전체 초안을 다시 보여주세요.\n\n"
)
...
async def call_tool(name: str, arguments: dict):
    ...
    # ================================================================
    if name == "tasklog":
        # [원격 차단 — 7/29] HTTP(원격 커넥터) 모드에서는 Git·토큰 수집을 하지 않는다.
        #   이 도구가 읽는 대상은 "서버 프로세스가 실행 중인 컴퓨터"의 저장소이지
        #   사용자의 컴퓨터가 아니다. 원격에서 그대로 실행하면 배포 서버의 커밋이
        #   사용자 업무일지에 사실처럼 실린다 → 지침 6번(확인할 수 없는 작업을
        #   사실처럼 작성하지 않는다) 정면 위반이라 부분 수집도 허용하지 않는다.
        #   _collect_today_token_usage()도 서버 계정의 ~/.claude 로그를 읽으므로
        #   같은 이유로 이 가드 뒤에 둔다(분기 1곳에서 모두 차단).
        #   조기 반환이므로 초안 캐시(_current_draft)도 건드리지 않는다.
        if _REQUEST_IS_HTTP.get():
            return [types.TextContent(type="text", text=(
                "⚠️ 원격 연결에서는 tasklog를 사용할 수 없습니다.\n\n"
                "이 도구는 Git 저장소를 직접 읽는데, 원격 서버가 읽을 수 있는 것은 "
                "사용자의 컴퓨터가 아니라 서버 자신의 저장소입니다. "
                "그대로 진행하면 본인이 하지 않은 작업이 업무일지에 기록됩니다.\n\n"
                "대신 chat_tasklog를 사용해 주세요. "
                "이 대화에서 실제로 확인된 작업만 근거로 초안을 만듭니다.\n"
                "Git 커밋 기반 초안이 필요하면 로컬 Claude Code에서 tasklog를 실행하세요."
            ))]
...
@mcp.tool(
    ...
    description=(
        ...
        "로컬 Claude Code 전용입니다. 원격 커넥터(HTTP) 연결에서는 사용자의 컴퓨터가 아니라 "
        "서버 자신의 저장소를 읽게 되므로 호출이 거부됩니다. 그 환경에서는 chat_tasklog를 사용하세요. "
